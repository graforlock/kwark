...
select.prototype.find = function (selector, all) {
    ...
    if (selected) this.node = selected;else console.log('select: ' + selector + ' is empty.');
    return this;
};
...
select.prototype.nodeify = function (target) {
    ...
    if (tags.length > 1) {
        ...
        tags.slice(1).map(function (attr) {
            var obj = {},
                attrs = attr.split('=');
            obj[attrs[0]] = attrs[1];
            return obj;
...
select.prototype.prepend = function (target) {
    target = target || document.body;
    if (target.children) target.insertBefore(this.node, target.children[0]);else target.appendChild(this.node);
    return this;
};
